...
def create_price_comparison_chart(lower_price, upper_price, current_price):
    """
    Tạo biểu đồ so sánh giá vé sử dụng Plotly
    ...
    if not all(isinstance(x, (int, float)) and x > 0 for x in [lower_price, upper_price, current_price]):
        st.warning("Dữ liệu giá không hợp lệ để tạo biểu đồ so sánh.")
        return

    # Khởi tạo figure
    fig = go.Figure()

    # Thêm thanh khoảng giá
    fig.add_trace(go.Bar(
        y=['Phạm vi giá'],
        x=[upper_price - lower_price],
        base=lower_price,
        marker_color='rgba(102, 178, 255, 0.7)',  # Xanh nhạt
        name='Khoảng giá trung bình',
        orientation='h',
        hoverinfo='text',
        hovertext=f'Khoảng giá: {lower_price:,.0f} - {upper_price:,.0f} VND'
    ))

    # Thêm vùng giá thấp hơn khoảng trung bình
    fig.add_trace(go.Bar(
        y=['Phạm vi giá'],
        x=[lower_price],
        base=0,
        marker_color='rgba(144, 238, 144, 0.6)',  # Xanh lá nhạt
        name='Giá thấp hơn',
        orientation='h',
        hoverinfo='text',
        hovertext=f'Dưới mức {lower_price:,.0f} VND'
    ))

    # Thêm vùng giá cao hơn khoảng trung bình
    fig.add_trace(go.Bar(
        y=['Phạm vi giá'],
        x=[upper_price],
        base=upper_price,
        marker_color='rgba(255, 99, 71, 0.6)',  # Đỏ nhạt
        name='Giá cao hơn',
        orientation='h',
        hoverinfo='text',
        hovertext=f'Trên mức {upper_price:,.0f} VND'
    ))
    ...
    fig.update_layout(
        ...
        title=dict(
            text='📊 So sánh giá vé với khoảng giá trung bình',
            xanchor="center",
            x=0.32,
            yanchor="top",
            font=dict(size=23, color='#2c3e50')
        ),
        xaxis=dict(
            title='Giá vé (VND)',
            showline=True,
...
def main():
    ...
    # Tiêu đề chính
    st.markdown("<h1 style='text-align: center; color: #2c3e50;'>✈️ Flight Booking Explorer 🌍</h1>", unsafe_allow_html=True)
    ...
    if source and destination and date and travel_class:
        ...
        if not flight_data.empty:
            ...
            with col_left:
                ...
                st.metric("🛬 Thời gian đến", selected_flight['Thời gian đến'])
        ...
        #     st.warning("Không có chuyến bay nào phù hợp với tiêu chí tìm kiếm.")     
            ...
            col_departure_weather, col_arrival_weather = st.columns(2)

            with col_departure_weather:
                with st.expander("🌦️ Thời tiết tại điểm khởi hành", expanded=False):
                    # Chuyển đổi thời gian khởi hành sang ngày
                    departure_date = pd.to_datetime(selected_flight['Thời gian khởi hành']).date()
                    
                    # Lấy thông tin thời tiết cho điểm đi
                    departure_weather_info = get_weather(source, departure_date)
                    
                    if departure_weather_info:
                        # Tạo các cột để hiển thị thời tiết
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            st.metric("🌡️ Nhiệt độ TB", f"{departure_weather_info['temperature']:.1f}°C")
                            st.metric("🌞 Nhiệt độ cao nhất", f"{departure_weather_info['max_temp']:.1f}°C")
                        
                        with col2:
                            st.metric("🌬️ Tốc độ gió", f"{departure_weather_info['max_wind']} km/h")
                            st.metric("🌡️ Nhiệt độ thấp nhất", f"{departure_weather_info['min_temp']:.1f}°C")
                        
                        st.metric("💧 Độ ẩm", f"{departure_weather_info['humidity']}%")
                        st.write(f"Mô tả: {departure_weather_info['condition']}")
                        
                        # Hiển thị biểu tượng thời tiết
                        st.image(departure_weather_info['icon'], width=100, caption="Biểu tượng thời tiết")
                    else:
                        st.warning("Không thể tải thông tin thời tiết điểm đi")

            with col_arrival_weather:
                with st.expander("🌦️ Thời tiết tại điểm đến", expanded=False):
                    # Convert arrival datetime to date
                    arrival_date = pd.to_datetime(selected_flight['Thời gian đến']).date()
                    
                    # Fetch weather for the destination on arrival date
                    weather_info = get_weather(destination, arrival_date)
                    
                    if weather_info:
                        # Create columns for weather display
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            st.metric("🌡️ Nhiệt độ TB", f"{weather_info['temperature']:.1f}°C")
                            st.metric("🌞 Nhiệt độ cao nhất", f"{weather_info['max_temp']:.1f}°C")
                        
                        with col2:
                            st.metric("🌬️ Tốc độ gió", f"{weather_info['max_wind']} km/h")
                            st.metric("🌡️ Nhiệt độ thấp nhất", f"{weather_info['min_temp']:.1f}°C")
                        
                        st.metric("💧 Độ ẩm", f"{weather_info['humidity']}%")
                        st.write(f"Mô tả: {weather_info['condition']}")
                        
                        # Display weather icon
                        st.image(weather_info['icon'], width=100, caption="Biểu tượng thời tiết")
                    else:
                        st.warning("Không thể tải thông tin thời tiết điểm đến")
